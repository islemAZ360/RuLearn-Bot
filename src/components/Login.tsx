...
import { Bot, Sparkles, Loader2, ExternalLink, Copy, Check, AlertTriangle } from 'lucide-react';
...
function isWebView(): boolean {
  ...
  if (/Android/.test(ua) && /Version\/[\d.]+/.test(ua) && !/Chrome\/[\d.]+/.test(ua)) {
    return true;
  }
  
  // More aggressive WebView detection for Android
...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setIsInWebView(true); // Show WebView instructions on failure
    } finally {
      setLoading(false);
    }
  };
            ...
            {isInWebView && (
              <div className="p-4 bg-amber-500/20 border border-amber-500/30 rounded-2xl text-amber-200 text-sm space-y-3">
                <div className="flex items-center gap-2 font-bold text-base">
                  ...
                  <span>تسجيل الدخول غير مدعوم هنا</span>
                </div>
                ...
                <div className="pt-2 space-y-2">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-3 bg-amber-600 text-white hover:bg-amber-500 transition-all duration-300 py-3 px-6 rounded-xl font-bold"
                  >
